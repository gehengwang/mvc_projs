...
<script type="text/javascript">
	...
	function formatLesson(value,row,index){
		if(row.questionStatus==1){
			return "<a href='/questionInit.do?lessonId="+row.lessonId+"&classId="+row.classId+"' style='color:#307DCA'>"+value+"</a>";
		}else{
			return value;
		}
	}
	...
	function onClickRow(index,row){
		if(row.questionStatus==1&&row.answerStatus==0){
			window.location.href = "/questionInit.do?lessonId="+row.lessonId+"&classId="+row.classId;
		}
	}
		...
		pageSize:20,url:query_url,onClickRow:onClickRow ">
			<thead>
				<tr>
					...
					<th data-options="field:'questionStatusName',align:'center',width:120">是否开放答题</th>
