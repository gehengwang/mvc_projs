...
$(function(){
	...
	//非管理员不能配班、配课、启停用状态、重置密码
	if('${sessionScope.USER_INFO.roles[0].roleId}'!=2){
...
function doQuery(){
	...
	var fmdata = {"classId":classId,"userName":userName};
...
function saveUserBind(){
	 ...
	 var fmdata = {"userIds":userIdArray.toString(),"classIds":classIdArray.toString(),"roleId":'3'};
...
function addTechLesson(){
	var row =  $("#data").datagrid("getSelected");
	if (row) {
		$("#lesson_data").datagrid("options").url="/queryLessonPage.do?techIdUse="+row.userId;
...
function saveTechLesson(){
	 ...
	 var fmdata = {"techIds":userIdArray.toString(),"lessonIds":lessonIdArray.toString()};
	 $.post("/saveTechLesson.do",fmdata,function(result){
		 if(result.success==true){
...
function lessonDetail(value,row,index){
	return "<a href='javascript:openLessonWin("+row.userId+")' style='color:#307DCA'>"+value+"</a>";
}

function openLessonWin(userId){
	$('#win_lesson').window('open');
	$("#lesson_data").datagrid("options").url="/queryLessonPage.do?techIdUse="+userId;
	$('#lesson_data').datagrid({"onLoadSuccess":function(dataExists){}});
}
...
function doImport(){
   ...
   if(d1==".xls"||d1==".xlsx"){
       fmSubmit("#importFm","/importUsers.do?roleId=3","#importBtn",function(){
   		$("#data").datagrid("load");
   	});
   }else{
...
function doExport(){
	var url="/userExport.do?flag=1&classId="+$("#class_id_query").combobox("getValue")+
...
<body>
	...
	<div id="toolbar" style="padding:5px;display:none">
		<form id="fm" method="post">
				...
				data-options="plain:true,iconCls:'icon-gears'" onclick="addUserClass()">老师配班</a>&nbsp;&nbsp;&nbsp;
				<a id="add_lesson" href="###" class="easyui-linkbutton" 
				data-options="plain:true,iconCls:'icon-gears'" onclick="addTechLesson()">老师配课</a>&nbsp;&nbsp;&nbsp;
	...
	<table id="data" class="easyui-datagrid" 
		data-options="fit:true,rownumbers:true,pagination:true,nowrap:false,toolbar:'#toolbar',
		pageSize:20,url:query_url,singleSelect:true">
			<thead>
				<tr>
					...
					<th data-options="field:'mobile',align:'center',width:100">手机号码</th>
					<th data-options="field:'sexName',align:'center',width:60">性别</th>
					<th data-options="field:'qq',align:'center',width:120">QQ</th>
					...
					<th data-options="field:'lessonName',align:'center',width:120,formatter:lessonDetail">课件详情</th>
				</tr>
			</thead>
	</table>
	...
	<div id="dlg_class" class="easyui-dialog"
			...
			style="width: 650px; height:80%;">
				...
				<tr>
					...
					<th data-options="field:'className',align:'center',width:220">班级名称</th>
	...
	<!-- 班级详情 -->
	<div id="win_class" class="easyui-window" title="班级详情" style="width: 650px; height:80%;"
	...
	<div id="dlg_lesson" class="easyui-dialog"
			data-options="iconCls:'icon-gears',closed:true,resizable:true,modal:true,title:'课程配置',
				buttons:[
				{ text:'保存',iconCls:'icon-upload',	handler:function(){saveTechLesson();}                   },
    ...
    <div id="win_lesson" class="easyui-window" title="课件详情" style="width:680px;height:80%"
        data-options="iconCls:'icon-save',modal:true,closed:true">
   		<table id="lesson_data" class="easyui-datagrid" 
   			data-options="fit:true,pagination:true,nowrap:false">
			<thead>
				<tr>
					<th data-options="field:'lessonId',align:'center',width:80">课件号</th>
					<th data-options="field:'lessonName',align:'center',width:260">课程名称</th>
					<th data-options="field:'techNameUse',align:'center',width:100">使用老师</th>
					<th data-options="field:'techNameOwn',align:'center',width:100">上传老师</th>
				</tr>
			</thead>
		</table>
